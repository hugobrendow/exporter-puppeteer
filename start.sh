...
for i in {1..30}; do
    if curl -f -s http://localhost:3000/health > /dev/null 2>&1; then
        ...
        echo ""
        echo "🧪 Testando API..."
        echo "Teste simples: curl http://localhost:3000/api/teste-pdf --output teste-simples.pdf"
        echo "Teste completo: curl -X POST http://localhost:3000/api/gerar-pdf -H 'Content-Type: application/json' -d '{\"produtos\":[{\"nome\":\"Teste\",\"preco\":\"10.99\",\"codigo\":\"123\",\"quantidade\":1}]}' --output teste.pdf"
        echo ""
        echo "📋 Comandos úteis:"
